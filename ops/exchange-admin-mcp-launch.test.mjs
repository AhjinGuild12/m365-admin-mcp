...
const CLI = path.join(HERE, 'exchange-admin-mcp-launch.mjs');
...
function validEnv(overrides = {}) {
  return {
    ...
    EXO_SECRET_EXPIRES: futureExpiry(60),
...
function makeWorkspace() {
  ...
  writeFile(path.join(root, 'packages/exchange-admin-mcp-server/src/exchange_admin_mcp/server.py'), 'print(1)\n', 0o644);
  writeFile(path.join(root, 'packages/exchange-admin-mcp-server/pyproject.toml'), '[project]\nname="exchange-admin-mcp"\n', 0o644);
  writeFile(path.join(root, 'packages/exchange-admin-mcp-server/uv.lock'), 'version = 1\n', 0o644);
  writeFile(path.join(root, 'packages/exchange-admin-mcp-server/.venv/pyvenv.cfg'), 'home = /usr\n', 0o644);
  writeFile(path.join(root, 'packages/exchange-admin-mcp-server/.venv/bin/python'), '#!/bin/sh\n', 0o755);
  ...
  writeFile(path.join(root, 'ops/exchange-admin-mcp-launch.mjs'), 'export {}\n', 0o644);
  writeFile(path.join(root, 'ops/m365-mcp-launch-core.mjs'), fs.readFileSync(path.join(HERE, 'm365-mcp-launch-core.mjs')));
  writeFile(path.join(root, 'ops/exchange-admin-mcp-gate.mjs'), 'export {}\n', 0o644);
...
test('CLI source never reads argv or env', () => {
  ...
  assert.match(src, /packages\/exchange-admin-mcp-server/);
  assert.match(src, /exchange_admin_mcp/);
});
...
test('valid env spawns with only teams vars, PATH, and locale', () => {
  ...
  launch(configDir, workspace, spawnImpl, {
    ...
    TEAMS_ADMIN_CLIENT_SECRET: 'teams-secret-value',
  });
  ...
  assert.equal(JSON.stringify(child).includes('teams-secret-value'), false);
});
...
test('past expiry dies before spawn', () => {
  const workspace = makeWorkspace();
  const configDir = makeConfig(workspace, {
    env: validEnv({ EXO_SECRET_EXPIRES: pastExpiry() }),
  });
  const { spawnImpl, spawned } = stubSpawn();
  const { error } = captureStderr(() => launch(configDir, workspace, spawnImpl));
  assert.equal(error instanceof LaunchError, true);
...
test('integrity mismatch against packages dies before spawn', () => {
  ...
  const configDir = makeConfig(workspace);
  fs.appendFileSync(
    path.join(workspace, 'packages/exchange-admin-mcp-server/src/exchange_admin_mcp/server.py'),
    '\n# changed\n',
  );
  const { spawnImpl, spawned } = stubSpawn();
  const { error } = captureStderr(() => launch(configDir, workspace, spawnImpl));
...
test('otherPrefixes names the four foreign workloads', () => {
  assert.deepEqual(DESC.otherPrefixes, ['ENTRA', 'INTUNE', 'SPO_ADMIN', 'TEAMS_ADMIN']);
  const pattern = new RegExp(`^(${DESC.otherPrefixes.join('|')})_`);
  for (const prefix of DESC.otherPrefixes) {
    assert.match(`${prefix}_CLIENT_SECRET`, pattern);
  }
});

test('certificate path does not satisfy a missing secret', () => {
  const workspace = makeWorkspace();
  const configDir = makeConfig(workspace, {
    env: validEnv({
      EXO_CLIENT_SECRET: '',
      EXO_CLIENT_CERTIFICATE_PATH: '/tmp/not-a-certificate.pem',
    }),
  });
  const { spawnImpl, spawned } = stubSpawn();
  const { error, stderr } = captureStderr(() => launch(configDir, workspace, spawnImpl));
  assert.equal(error instanceof LaunchError, true);
  assert.match(stderr, /certificate path does not satisfy a missing secret/);
  assert.equal(spawned.length, 0);
});
