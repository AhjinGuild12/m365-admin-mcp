...
def test_get_rejects_non_graph_origin(graph_client_factory) -> None:
    client = graph_client_factory(
        http_client=httpx.Client(transport=httpx.MockTransport(lambda r: httpx.Response(200, json={})))
    )
    with pytest.raises(SanitizedGraphError, match="origin_rejected"):
        client.get("/organization", origin="reports")
    with pytest.raises(SanitizedGraphError, match="origin_rejected"):
        client.get("/organization", origin="login")


def test_get_accepts_graph_origin(graph_client_factory) -> None:
    client = graph_client_factory(
        http_client=httpx.Client(
            transport=httpx.MockTransport(lambda r: httpx.Response(200, json={"ok": True}))
        )
    )
    assert client.get("/organization", origin=Origin.GRAPH) == {"ok": True}
    assert client.get("/organization", origin="graph") == {"ok": True}
