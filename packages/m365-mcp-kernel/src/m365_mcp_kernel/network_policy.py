...
Copied from entra_mcp.network_policy then parameterized: the allowed
network-capable module set is an argument; default is graph_client.py only.
"""
...
DEFAULT_ALLOWED_NETWORK_FILES = frozenset({"graph_client.py"})
...
class _ImportVisitor(ast.NodeVisitor):
    ...
    def _allow_network(self, name: str) -> bool:
        ...
        if name.startswith("azure.identity"):
            return True
        if name == "urllib.parse" or name.startswith("urllib.parse"):
            return True
        return False
...
def check_tree(
    root: Path,
    *,
    allowed_network_files: frozenset[str] | None = None,
) -> list[PolicyViolation]:
    allowed = allowed_network_files if allowed_network_files is not None else DEFAULT_ALLOWED_NETWORK_FILES
    violations: list[PolicyViolation] = []
    for path in sorted(root.rglob("*.py")):
        source = path.read_text(encoding="utf-8")
        tree = ast.parse(source, filename=str(path))
        is_graph_client = path.name in allowed
